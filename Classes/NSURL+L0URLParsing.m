...
- (NSDictionary*) dictionaryByDecodingQueryString;
{
	...
	for (NSString* pair in keyValuePairs) {
		...
		if (value)
			[dict setObject:[value stringByRemovingPercentEncoding] forKey:[key stringByRemovingPercentEncoding]];
		else
			[dict setObject:[NSNull null] forKey:[key stringByRemovingPercentEncoding]];
...
- (NSString*) queryString;
{
	...
	for (NSString* key in self) {
		...
		[queryString appendString:[key stringByRemovingPercentEncoding]];
		
		id value = [self objectForKey:key];
		if (![value isEqual:[NSNull null]]) {
			[queryString appendString:@"="];
			[queryString appendString:[value stringByRemovingPercentEncoding]];
		}
